...
for item in itemList:
    ...
    if craftingTable and re.search("Recipes*", str(soup.find("h3").find("span", class_="mw-headline"))):
        ...
        for row in rows[1::]:
            ...
            craftDict["craft_id"] = str(craftId)
            ...
            if row.find("td", class_="result"):
                ...
                if not qty:
                    craftDict["craft_qty"] = '1'
                else:
                    craftDict["craft_qty"] = ' '.join(re.findall("\((\d+)\)", qty.text))
            else:
                craftDict["craft_qty"] = craftsList[-1]["craft_qty"]
            ...
            for stationInstance in stationList:
                if not stationInstance == "By Hand":
                    craftDict["table"].append(stationInstance.span.span.a.text)
                else:
                    craftDict["table"].append(stationInstance)

            craftsList.append(craftDict)
            craftId += 1
